...
- Dashboard auth: **Implemented in code (`apps/dashboard`).** The operator console is now a separate Worker that reaches the gateway over a private RPC service binding (never public HTTP) and re-verifies a Cloudflare Access JWT on every request (`apps/dashboard/src/access.ts`). Public requests fail closed unless `ACCESS_TEAM_DOMAIN` + `ACCESS_AUD` are set and valid; localhost development is the only no-Access exception. The remaining step is enabling the account-level Zero Trust Access application (`eccos-45t`), so do not expose the dashboard until that is done.
