...
function setTenantMode(value: "false" | "shadow" | "true") {
  (env as MutableEnv).ECCOS_MULTI_TENANT = value;
}

function setMultiTenant(value: boolean) {
  setTenantMode(value ? "true" : "false");
}
...
function accountApi(apiKey: string, origin = "http://example.com") {
...
async function registerWabaHttp(
  ...
  return adminJson(`/v1/accounts/${accountId}/wabas`, {
    wabaId,
    metaAccessToken,
    phones,
    ...subscriber,
...
describe("multi-tenant control plane", () => {
  ...
  it("lets two accounts own multiple WABAs/phones and rejects duplicate ownership without mutation", async () => {
    ...
    await cp((i) => {
      i.registerWaba({ accountId: accA.accountId, wabaId: "WABA_A1", metaAccessToken: "tok-a1", phones: [{ phoneNumberId: "PN_A1_1", displayPhoneNumber: "+34 600 000 001" }] });
      i.registerWaba({ accountId: accA.accountId, wabaId: "WABA_A2", metaAccessToken: "tok-a2", phones: [{ phoneNumberId: "PN_A2_1", displayPhoneNumber: "+34 600 000 002" }] });
      i.registerWaba({ accountId: accB.accountId, wabaId: "WABA_B1", metaAccessToken: "tok-b1", phones: [{ phoneNumberId: "PN_B1_1", displayPhoneNumber: "+34 600 000 003" }] });
      i.registerWaba({ accountId: accB.accountId, wabaId: "WABA_B2", metaAccessToken: "tok-b2", phones: [{ phoneNumberId: "PN_B2_1", displayPhoneNumber: "+34 600 000 004" }] });
    });

    await expect(
      cp((i) => i.registerWaba({ accountId: accB.accountId, wabaId: "WABA_A1", metaAccessToken: "tok-x", phones: [{ phoneNumberId: "PN_X" }] })),
    ).rejects.toThrow(/already registered to another account/);

    await expect(
      cp((i) => i.registerWaba({ accountId: accB.accountId, wabaId: "WABA_B2", metaAccessToken: "tok-x", phones: [{ phoneNumberId: "PN_A1_1" }] })),
    ).rejects.toThrow(/already registered to another account/);

    await expect(
      cp((i) =>
        i.registerWabas([
          { accountId: accB.accountId, wabaId: "WABA_B3", metaAccessToken: "tok-b3", phones: [{ phoneNumberId: "PN_B3_1" }] },
          { accountId: accB.accountId, wabaId: "WABA_B4", metaAccessToken: "tok-b4", phones: [{ phoneNumberId: "PN_A1_1" }] },
        ]),
      ),
    ).rejects.toThrow(/already registered to another account/);
  ...
  it("preserves the existing WABA shard through HTTP shadow migration and legacy rollback", async () => {
    setMultiTenant(false);
    await runInDurableObject(getGatewayStubForWaba(env, "WABA_MIGRATE"), async (gateway: EccosGateway) => {
      gateway.saveConfig({
        ...
        SUBSCRIBER_WEBHOOK_URL: "https://legacy.example/webhook",
        SUBSCRIBER_SECRET: "legacy-secret",
      });
      gateway.ingest([
        {
          type: "reply",
    ...
    const registration = await registerWabaHttp(
      ...
      { subscriber_webhook_url: "https://legacy.example/webhook", subscriber_secret: "legacy-secret" },
    );
    expect(registration.status).toBe(201);

    const graph = mockGraph();
    const legacySend = await exports.default.fetch("http://example.com/v1/wabas/WABA_MIGRATE/messages", {
      method: "POST",
      headers: { authorization: `Bearer ${env.ECCOS_API_KEY}`, "content-type": "application/json" },
      body: JSON.stringify({ to: "34600000401", type: "text", text: { body: "legacy remains live" } }),
    });
      ...
      entry: [{
        ...
        changes: [{
          ...
          value: {
            ...
            messages: [{ from: "34600000402", id: "wamid.MIGRATE_SHADOW", timestamp: "1700000000", type: "text", text: { body: "shadow remains live" } }],
          },
        }],
      }],
    });
    ...
    setMultiTenant(false);
    expect((await rpc.getStatus("WABA_MIGRATE")).connection.wabaId).toBe("WABA_MIGRATE");
    expect(await rpc.getSubscriberConfig("WABA_MIGRATE")).toEqual({
      url: "https://legacy.example/webhook",
      hasSecret: true,
    });
  });
  ...
  it("maps an OAuth connect state to the initiating account and consumes it once", async () => {
    ...
    await cp((i) => {
      expect(i.consumeConnectState(state)).toBe(accountId);
      expect(i.consumeConnectState(state)).toBeNull();
    });
  ...
  it("filters multi-tenant webhooks by registered WABA and phone", async () => {
    setMultiTenant(true);
